...
The AppImage is copied atomically to `~/.local/share/vulcan/app/Vulcan.AppImage`. Desktop metadata is written under the user's XDG directories. uv/Python/Etna/Vulcan state is under `~/.local/share/vulcan`. Docker is reused when healthy; on Arch the missing-engine path is `pacman -S docker`, while other Linux hosts use Docker's installation script. Privileged host operations use Polkit (`pkexec`).
