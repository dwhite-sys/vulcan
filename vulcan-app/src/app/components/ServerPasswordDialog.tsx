...
import { setServerPasswordPromptHandler, type ServerPasswordPrompt } from '../services/serverPasswordPrompt';

type Resolver = (password: string) => void;
...
export function ServerPasswordDialog() {
  ...
  useEffect(() => setServerPasswordPromptHandler((next) => new Promise<string>((resolve, reject) => {
    setDetails(next);
    setPassword('');
    setResolver(() => resolve);
    setRejecter(() => reject);
    ...
    <div className="fixed inset-0 z-[101] flex items-center justify-center bg-black/60 backdrop-blur-sm p-6">
      <form onSubmit={submit} className="w-full max-w-[440px] overflow-hidden rounded-[14px] border border-ash-700 bg-ash-900 shadow-2xl">
        ...
        <div className="border-t border-ash-800 px-[22px] py-3.5 text-xs text-ash-500">
          The password is sent only inside this encrypted connection and is not stored by the frontend.
        </div>
      </form>
    </div>
  );
