  ...
  const popup = isOpen && createPortal(
    ...
    >
      ...
      <div className="max-h-80 overflow-y-auto">
        {skills.length === 0 ? (
          <div className="p-4 text-center text-ash-500 text-sm">No general skills installed</div>
        ) : (
          <div className="p-1">
            {skills.map((skill) => (
              <div
              ...
              >
                ...
                >
                  <div className={`w-10 h-5 rounded-full transition-colors ${skill.enabled ? 'bg-coral-500' : 'bg-ash-700'}`}>
                    <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${skill.enabled ? 'translate-x-5' : 'translate-x-0.5'}`} />
                  </div>
                </button>
              </div>
            ))}
