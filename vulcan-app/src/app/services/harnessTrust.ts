...
export async function requireTrustedHarness(url: string, identity: Uint8Array): Promise<void> {
  ...
  const request = (async () => {
    ...
    if (!handler) {
      throw new Error('Harness identity requires approval, but the trust dialog is unavailable.');
    }
    ...
    const decision = await handler(details);
    ...
    if (decision.remember) localStorage.setItem(pinKey, encoded);
  })();
