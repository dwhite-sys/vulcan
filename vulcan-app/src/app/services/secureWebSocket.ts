...
export class SecureWebSocket {
  ...
  private ready = false;
  // Web Crypto operations are asynchronous. Without serialization, two calls
  // to send() can observe the same sequence number before either encryption
  // finishes, producing duplicate/non-monotonic secure frames and causing the
  // server to close the socket. Likewise, browser message callbacks can overlap
  // while decrypting. Keep both directions strictly ordered.
  ...
  send(payload: JsonValue): Promise<void> {
    const operation = this.sendChain.then(() => this.sendOrdered(payload));
    // Keep the queue usable after an individual send failure while still
    // returning that failure to the caller that initiated it.
    this.sendChain = operation.catch(() => {});
    return operation;
  }

  private async sendOrdered(payload: JsonValue): Promise<void> {
