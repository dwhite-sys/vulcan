...
export async function gitCommit(chatId: string, message: string): Promise<string> {
  const data = await wsRequest('git/commit', { chat_id: chatId, message });
  return data.hash ?? '';
}
...
export async function hasChangedSinceLastCommit(_chatId: string, _path: string): Promise<boolean> {
  return true;
}
...
export async function ensureContainer(chatId: string, globalChat = false): Promise<void> {
  await wsRequest('container/start', { chat_id: chatId, global_chat: globalChat });
}
...
export async function remoteLoadChats(): Promise<any[]> {
  const data = await wsRequest('chats/list', {});
  return data.chats ?? [];
}
...
async function uploadFileBytes(chatId: string, file: File, options: UploadOptions = {}): Promise<string> {
  ...
  const removeProgress = () => emitTransfer({ id: transferId, remove: true });
  const assertNotCancelled = () => { if (cancelled) throw new VulcanTransferCancelledError(); };

  emitTransfer({ id: transferId, filename: file.name, direction: 'upload', preparing: true, percent: 0 });
  try {
    assertNotCancelled();
    ...
    assertNotCancelled();
    emitTransfer({ id: transferId, filename: file.name, direction: 'upload', preparing: false, percent: file.size === 0 ? 100 : 0 });
    ...
    assertNotCancelled();
    removeProgress();
    emitTransferComplete({ id: transferId, filename: file.name, direction: 'upload' });
...
export async function openSlot(chatId: string, kind: SlotKind): Promise<number> {
  const data = await wsRequest('terminal/slot/open', { chat_id: chatId, kind });
  return data.slot;
}
...
export async function runCommandInSlot(chatId: string, kind: SlotKind, slot: number, cmd: string, timeout = 180): Promise<string> {
...
export async function sendSlotInput(chatId: string, kind: SlotKind, slot: number, text: string): Promise<void> {
  // Input goes directly over the terminal WS — this REST path is not used
  // TerminalSlotWidget sends input via its own WS connection
}
