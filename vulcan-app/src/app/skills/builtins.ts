...
export interface BuiltinSkill {
  ...
  body: string;
  source: 'vulcan';
  isActive: (settings: { cliWorkspaceEnabled: boolean; panelsEnabled: boolean }) => boolean;
}
...
export function getActiveBuiltinSkills(settings: { cliWorkspaceEnabled: boolean; panelsEnabled: boolean }): BuiltinSkill[] {
  return BUILTIN_SKILLS.filter((skill) => skill.isActive(settings));
}
