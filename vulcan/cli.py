...
def cmd_install(args):
    ...
    if docker.image_exists():
        ...
        print()
        info("Building Docker workspace image (first time only, ~30s)...")
    ...
    if docker.global_image_exists():
        info("Global environment image already built")
    else:
        info("Building global environment image (first time only)...")
        if docker.build_global_image(verbose=args.verbose):
            ok("Global environment image built")
        else:
            warn("Global environment image could not be built; Vulcan itself remains usable")
...
def cmd_status(args):
    ...
    info(f"Kit server: {config.get('etna_url', 'not set')}")
    print()
...
def main():
    ...
    # install
    p_install = sub.add_parser("install", help="First-time setup")
