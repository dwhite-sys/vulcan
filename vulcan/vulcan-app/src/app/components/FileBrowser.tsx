...
function ContextMenu({ x, y, node, chatId, onRename, onDelete, onDownload, onClose }: ContextMenuProps) {
  ...
  return (
    ...
    >
      {[
        ...(!node.isDir ? [{ label: 'Attach file', action: () => window.dispatchEvent(new CustomEvent('vulcan:file-reference', {
          detail: { chatId, path: node.path },
        })) }] : []),
...
export function FileBrowser({ chatId, onOpenFile }: FileBrowserProps) {
  ...
  const load = useCallback(async () => {
    // Recursive workspace enumeration can be expensive on remote/large trees.
    // Never overlap polls; every caller joins the current scan instead.
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-ash-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={async () => {
                try {
                  const path = workspacePathRef.current ?? await getWorkspacePath(chatId);
