...
import { insertQuoteSourceMarks, type QuoteMarkupNode, type QuoteSourceMark } from '../services/quoteGeometry';
import { codeChildrenToString } from '../services/markdownDefense';

function rehypeQuoteSourceMarks(options?: { marks: QuoteSourceMark[] }) {
  return (tree: QuoteMarkupNode) => {
    if (options?.marks.length) insertQuoteSourceMarks(tree, options.marks);
  };
}

export function MarkdownRenderer({ content, sourceMarks = [] }: { content: string; sourceMarks?: QuoteSourceMark[] }) {
  return (
    ...
    ">
      <ReactMarkdown
        ...
        rehypePlugins={sourceMarks.length ? [[rehypeQuoteSourceMarks, { marks: sourceMarks }]] : undefined}
        components={{
          ...
          code({ className, children, ...props }: any) {
            ...
            const codeStr = codeChildrenToString(children);
            if (match) {
              // An empty fenced block is provider/Markdown noise, not a code artifact.
