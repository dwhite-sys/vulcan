...
import { getModelDisplayName } from '../services/modelVision';
...
export function ModelSelector({ selectedModel, selectedProviderId, onSelectModel, isConfigured }: ModelSelectorProps) {
  ...
  useEffect(() => {
    const onCatalogUpdated = () => setCatalogRevision((value) => value + 1);
    window.addEventListener('vulcan:model-catalog-updated', onCatalogUpdated);
    return () => window.removeEventListener('vulcan:model-catalog-updated', onCatalogUpdated);
  }, []);
  ...
  const formatModelName = (model: ModelInfo): string => {
    // Prefer the exact models.dev record when one exists. This gives Vulcan a
    // stable human-readable name without mutating or guessing the provider ID.
    const catalogName = getModelDisplayName(model.id);
  ...
  const selectedDisplay = selectedModel
    ? (() => {
        // Catalog metadata should improve the selected label even before the
        // dropdown has been opened and the provider model list has loaded.
        const catalogName = getModelDisplayName(selectedModel);
        if (catalogName) return catalogName;
