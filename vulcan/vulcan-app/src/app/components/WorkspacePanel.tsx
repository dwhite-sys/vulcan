...
import { FileText, FileCode, Image, File, Search, X, Download, LayoutDashboard, Monitor } from 'lucide-react';
...
import type { PresentedFile, DesignAttachment, MessageAttachment, Panel as PanelMeta, TerminalSlotMeta, SlotKind } from '../types/vulcan';
...
function canOpenInWorkspace(name: string): boolean {
  const ext = name.split('.').pop()?.toLowerCase() ?? '';
  return !['pdf', 'zip', 'tar', 'gz'].includes(ext);
}
...
function ContentTab({ chatId, presentedFiles, panels, designs, openDesignId, attachments, onOpenFile, onOpenPanel, onOpenDesign }: ContentTabProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
        ...
        <div className="px-2 pb-2">
          {filteredArtifacts.map((file, idx) => (
            <div key={idx} className="group flex items-center w-full hover:bg-ash-800/60 transition-colors rounded">
              <button
                onClick={() => canOpenInWorkspace(file.name) ? onOpenFile(file.path, file.name) : undefined}
      ...
      <div className="px-3 pt-2 pb-1 border-t border-ash-800/50 flex-shrink-0">
        <span className="text-xs font-semibold text-ash-400 uppercase tracking-wide">
          Designs
          {designs.length > 0 && <span className="ml-1 text-ash-600">({designs.length})</span>}
        </span>
      </div>
      {designs.length === 0 ? (
        <div className="px-3 py-2 text-xs text-ash-600">No designs yet</div>
      ) : (
        <div className="px-2 pb-2">
          {designs.map((design) => (
            <div key={design.id} className={`group flex items-center w-full transition-colors rounded ${design.id === openDesignId ? 'bg-ash-800/80' : 'hover:bg-ash-800/60'}`}>
              <button
                onClick={() => onOpenDesign(design.name)}
                className="flex items-center gap-1.5 flex-1 text-left py-1 min-w-0 pl-1"
              >
                <Monitor className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                <span className="text-xs font-mono text-ash-300 truncate">{design.name}</span>
              </button>
            </div>
          ))}
        </div>
...
export function WorkspacePanel({
  ...
  const [slotStatuses, setSlotStatuses] = useState<Record<string, TerminalSlotMeta['status']>>({});
  const [terminalHeight, setTerminalHeight] = useState(220);
  const terminalPanelRef = useRef<HTMLDivElement>(null);
  const terminalHandleRef = useRef<HTMLDivElement>(null);

  const handleSlotStatus = useCallback((kind: SlotKind, slot: number, status: TerminalSlotMeta['status']) => {
  ...
  const userSlotCount = terminalSlots.filter((s) => s.kind === 'user' && s.status !== 'closed-inactivity').length;
